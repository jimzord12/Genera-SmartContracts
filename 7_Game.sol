...
contract GameManager is ERC1155, Ownable {
    ...
    struct Card {
        ...
        uint template_id; // Card's Type ID -> 1 => Wind Turbine
        bool inMP; // stands for: placed in Marketplace for sale
        address owner;
    }

    mapping(address => Player) public players;
    //Server's card ID => Card Instance
    ...
    */
    function createCard(uint _card_id, uint _template_id, bool _inMP) public {
        require(cards[_card_id].nft_id == 0, "Card already exists");
        require(_template_id > 0 && _template_id < 100, "Parameter: _template_id, is invalid");

        cards[numCards] = Card(numCards, _card_id, _template_id, _inMP, msg.sender); // Creates a Card instance
    ...
    */
    function transferCard(address _seller, address _buyer, uint _cardId /*, uint _amount*/ ) public {
        ...
        Card storage card = cards[_cardId]; // Get the Card (we need the NFT's ID) using its Server ID
        safeTransferFrom(_seller, _buyer, card.nft_id, 0/* amount */, ""); // handles the ERC-1155 NFT magic
        uint256[] storage _sellerCards = ownedTokens[_seller]; // Get/Select Seller's Cards
    ...
    */
    function sellCard(uint _card_id) public {
        Card storage card = cards[_card_id]; // Get the Card (we need the NFT's ID) using its Server ID
